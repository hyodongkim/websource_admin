	...
	var nTotalSize = 0;
	var nMaxImageSize = 100*1024*1024;
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/saadmin/editor/uploadForEditor.do',	//샘플 URL입니다.
 	        sCallback : 'se/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
...
// 	    			sMsgNotAllowedExt (String) 허용되지 않는 파일 형식인 경우 띄워줄 경고메시지
// 	    		}
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	window.onload = function(){
  		checkDragAndDropAPI();
  		bSupportDragAndDropAPI = false; // 2014-05-27 김도현
  		if(bSupportDragAndDropAPI){
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 		//alert('setPhotoToEditor');
 		//alert(oFileInfo[0].sFileName);
 		var savedName = oFileInfo[0].sFileName;
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', ["<img src='../imgview.do?savedName="+savedName+"' alt='첨부된 이미지'/>"]);
		}
	}
