          ...
          <tr>
            ...
            <td>
              <select name="type" id="type">
                <option value="온라인" <c:if test="${item.type == '온라인'}">selected</c:if>>온라인</option>
                <option value="오프라인" <c:if test="${item.type == '오프라인'}">selected</c:if>>오프라인</option>
                <option value="기타" <c:if test="${item.type == '기타'}">selected</c:if>>기타</option>
              </select>
            </td>
          </tr>
          <tr>
            <th scope="row" style="width: 120px;">판매처명</th>
            <td>
              <input type="text" name="name" id="name" value="${item.name}">
            </td>
          </tr>
          <tr>
            <th scope="row" style="width: 120px;">판매처코드</th>
            <td>
              <input type="text" name="code" id="code" value="${item.code}"></td>
          </tr>
          <tr>
            <th scope="row" style="width: 120px;">칼라</th>
            <td>
              <input type="text" name="color" id="color" value="${item.color}"> [rgb(255,255,0)등의 형식으로 입력해주시기 바랍니다.]</td>
          </tr>
          <tr>
            ...
            <td>
              ...
              <span style="color: gray">첨부파일 용량은 50MB까지만 가능합니다. (권장 이미지 사이즈 :300픽셀 * 300픽셀)</span>
              <div class="file_area1">
                <c:if test="${! empty item}">
                  ...
                  <p>
                    <img src="${path}/imgview.do?id=${item.files_id}" style="width: 300px; border-radius: 10px;" onerror="this.style.display='none';"/>
                  </p>
                  <p><button type="button" onclick="window.open('/saadmin/util/download.do?id=${item.files_id}');">로고 다운로드</button> </p>
                </c:if>
              </div>
            </td>
          </tr>
