      ...
      <div class="table-responsive">
        <table class="table table-bordered dataTable" width="100%" cellspacing="0">
          ...
          <tr>
            ...
            <td>
              <input type="text" name="code" id="code" value="${item.code}"> (코드를 넣지 않으면 전체 카테고리 아이콘입니다.)
            </td>
          </tr>
          <tr>
            <th scope="row" style="width: 120px;">노출순서</th>
            <td>
              <input type="text" name="visible_order" id="visible_order" value="${item.visible_order}"> * 자연수만 입력하십시오!
            </td>
          </tr>
          <tr>
            ...
            <td>
              ...
              <span style="color: gray">대표 이미지는 50MB까지만 올릴 수 있습니다.</span>
              <div class="file_area1">
                <c:if test="${! empty item}">
                  ...
                  <c:if test="${fn:endsWith(item.original_name,'svg')}">
                    <iframe src="<c:out value="${path}/imgview.do?id=${item.files_id}"/>" width="100px" height="100px"></iframe>
                  </c:if>
...
<script>
  function go_reg(){
    ...
    if($("#code").val()==""){
      if(!confirm('카테고리 코드를 넣지 않으면, 전체 카테고리로 인식합니다.')){
        return false;
      }
//      alert('카테고리코드를 입력하세요.');
//      return;
