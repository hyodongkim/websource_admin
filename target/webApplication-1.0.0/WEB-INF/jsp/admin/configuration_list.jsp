...
<script>
    ...
    function go_delete_configuration_checked(){
        ...
        $(".checked:checked").each(function(){
            let id = $(this).val();
            $.post("${path}/deleteConfigurationAction.do", {
                'id' : id
            }, function(result){
                if (result == "SUCCESS"){
    ...
    function go_delete_configuration(id){
...
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">환경설정 관리</h1>
...
<div class="card shadow mb-4">
    ...
    <div class="input-group-append" style="float: left; padding: 15px;">
        <%--<button class="btn btn-warning" type="button" onclick="go_delete_configuration_checked()">
            선택삭제
        </button>--%>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered dataTable striped" width="100%" cellspacing="0">
                ...
                <c:choose>
                    <c:when test="${empty list}">
                        <tr>
                            <td colspan="12">등록한 환경설정이 없습니다.</td>
                        </tr>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="item" items="${list}" varStatus="status">
                            <tr>
                                ...
                                <td>
                                    <a href="${path}/configurationBoth.do?id=<c:out value="${item.id}"/>" class="btn btn-primary btn-icon-split btn-sm"><span class="text">수정</span></a>
                                    <a href="javascript:go_delete_configuration('<c:out value="${item.id}"/>');" class="btn btn-danger btn-icon-split btn-sm"><span class="text">삭제</span></a>
                                </td>
                            </tr>
                        </c:forEach>
                    </c:otherwise>
            ...
            <div>
                <a href="${path}/configurationBoth.do" class="btn btn-primary btn-icon-split btn-sm"><span class="text">환경설정 등록</span></a>
            </div>
        </div>
    </div>
</div>
