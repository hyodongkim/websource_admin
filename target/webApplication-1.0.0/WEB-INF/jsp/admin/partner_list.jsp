...
<script>
    ...
    function go_delete_partner_checked(){
        ...
        $(".checked:checked").each(function(){
            let pk_partner = $(this).val();
            $.post("${path}/saadmin/deletePartnerAction.do", {
    ...
    function go_delete_partner(pk_partner){
    ...
    function change_process_yn(pk_partner, obj){
        if(!confirm('처리여부를 변경하시겠습니까?')){
            return false;
        }
        $.post("${path}/saadmin/changePartnerProcessAction.do", {
            'pk_partner': pk_partner,
...
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">파트너 요청 목록</h1>
...
<div class="card shadow mb-4">
    ...
    <div class="input-group-append" style="float: left; padding: 15px;">
        <button class="btn btn-warning" type="button" onclick="go_delete_partner_checked()">
            선택삭제
        </button>
    </div>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:forEach var="item" items="${list}" varStatus="status">
                            <tr>
                                <td><input type="checkbox" value="${item.pk_partner}" class="checked"/></td>
                                ...
                                <td><select onchange="change_process_yn('<c:out value="${item.pk_partner}"/>',this);">
                                ...
                                <td>
                                    <a href="${path}/saadmin/viewPartner.do?pk_partner=<c:out value="${item.pk_partner}"/>" class="btn btn-primary btn-icon-split btn-sm"><span class="text">상세</span></a>
                                    <a href="javascript:go_delete_partner('<c:out value="${item.pk_partner}"/>');" class="btn btn-danger btn-icon-split btn-sm"><span class="text">삭제</span></a>
                                </td>
                            </tr>
                        </c:forEach>
                    </c:otherwise>
