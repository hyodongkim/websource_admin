...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
	...
	$ON_SET_PHOTO : function(aPhotoData){
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				this._getPhotoTagCallback(aPhotoInfo);
			}
	 ...
	 * @use 일반 포토 tag 생성
	 */
	_getPhotoTagCallback : function(htPhotoInfo){
		// id와 class는 썸네일과 연관이 많습니다. 수정시 썸네일 영역도 Test
		...
		img.onload = function() {
			...
			var sThumb = "";
			var sTag = '<img src="' + sThumb + '{=sOriginalImageURL}" title="{=sName}" width="' + width + '">';
			if(true || htPhotoInfo.bNewLine){
				sTag += '<br style="clear:both;">';
				sTag += '<br style="clear:both;">';
			}
			sTag = jindo.$Template(sTag).process(htPhotoInfo);
			oApp.exec("PASTE_HTML", [sTag]);
		};
		img.src = root + htPhotoInfo.sOriginalImageURL + "?dummy=" + (new Date().getTime());
	}
});